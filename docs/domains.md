...
The Domains tool maps over the [Domain OpenSIPS module](https://docs.opensips.org/manual/2-3/modules/domain). The tools is used to perform modifications in OpenSIPS's local domain list during runtime. The domain names are kept in a database and they can be re-loaded into OpenSIPS via OCP.
