...
The Statistics Monitor tool is an viewer for the statistics provided by OpenSIPS via the [Statistics Inteface](https://docs.opensips.org/manual/2-2/interface-statistics). The tool is able to display realtime statistic values, but also to sample and to draw charts for the certain statistics (value history).
