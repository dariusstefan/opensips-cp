...
The OCP dialplan tool maps over the [Dialplan OpenSIPS module](https://docs.opensips.org/manual/2-4/modules/dialplan). The tool is used to perform modifications in OpenSIPS's dialplan rules during runtime. The rules are kept in a database and they can be reloaded into OpenSIPS from the web interface (see the *Apply Changes to Server* button).
